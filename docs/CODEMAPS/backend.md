...
라우터 8개가 `/api` 접두사로 붙는다.

| 라우터 | 접두사 | 인증 | 책임 |
|---|---|---|---|
| `routes_auth` | `/auth` | 부분 | 가입(초대 필수)·로그인·로그아웃·현재 사용자 |
| `routes_admin` | `/admin` | 관리자 | 초대 코드 발급·목록·회수 |
| `routes_files` | `/files` | 필요 | 작업공간 트리·읽기·쓰기·폴더·이름변경·삭제·**실행 제출** |
| `routes_jobs` | (없음) | 필요 | 잡 상세·중단·산출물 원문, 예전 프로젝트 기반 제출 |
...
정책과 저장소를 분리했다. `policy.py` 는 순수 규칙(동시 10명, 유휴 30분,
...
- 정원 계산은 SCAN. 활성 세션이 최대 11개라 인덱스를 따로 두는 것보다
  TTL 과 어긋날 위험이 없는 쪽이 낫다.
...
**DB 를 큐로 쓴다.** 별도 브로커가 없는 이유는 규모가 작고(동시 4, 사용자 10)
...
- `sweeper.py` / `cache.py` — 산출물 청소. 로그아웃은 눌러야 일어나는데 브라우저를
  그냥 닫으면 아무 일도 없다. 그래서 워커가 주기적으로 "지금 화면을 보고 있지
  않은" 사용자의 `.str` 을 비운다. **관리자의 유휴 면제를 여기까지 끌고 오지
  않는다** — 그러면 관리자 산출물이 영영 정리되지 않는다. 로그는 지우지 않고,
  아직 끝나지 않은 잡의 디렉토리도 건드리지 않는다.
